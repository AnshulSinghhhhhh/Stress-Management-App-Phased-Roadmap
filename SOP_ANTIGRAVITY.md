...
- **The frontend Supabase client (`frontend/src/api/supabase.ts`) is auth-only — it must never gain `.from()` table access.** All reads/writes to `checkins`, `crisis_events`, `relief_sessions`, and `trigger_tags` must go exclusively through FastAPI, because that's the only place the synchronous crisis detector runs before a check-in is persisted. This is enforced with a TypeScript type guard exposing only `.auth` from the client, not the full Supabase client type — if an agent's diff widens that type or adds a `.from()` call anywhere in the frontend, stop and review before accepting, since it would let a check-in bypass crisis detection entirely.
